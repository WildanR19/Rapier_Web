              ...
              <tbody>
                @foreach ($category as $cat)
                    <tr>
                        ...
                        <td><a href="{{ route('task.category.delete', $cat->id) }}" class="btn btn-sm btn-outline-danger">Remove</a></td>
                    </tr>
                @endforeach
              </tbody>
            </table>
            <form action="{{ route('task.category.add') }}" method="post" id="formCat">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="inputcategory">Category Name</label>
                            <input type="text" id="inputcategory" name="category_name" class="form-control">
                        </div>
                    </div>
                </div>
            </form>
